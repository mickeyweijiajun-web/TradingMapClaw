...
## [v2.0] — 2026-07-12 — 活跃开发中

### 双引擎委员会正式确立

委员会结构正式确立为**双引擎架构**（Pass A / B / C），并明确权重分配——Pass A（Hermes/GLM-5.2：基本面、估值、内部人，40%）、Pass B（Codex/GPT-5.6：技术面、资金流、期权、交叉核验，35%）、Pass C（Hermes/GLM-5.2：宏观、行业、情绪、监管，25%）——并配备了针对争议判断的**委员会作战室**决胜协议（DeepSeek → GLM-5.2 → GPT-5.6），建立在现有的全部必须完成关卡之上。

### 两轮联合审计——累计修复 14 个 bug

在 v1.6 审计基础上，又进行了两轮 Hermes+Codex 联合审计，累计修复达到 **14 个 bug**，其中包括一个严重的 `HERMES_HOME` 未定义变量 bug，该 bug 一直在静默禁用 Pass C 的记忆功能。

### 规模与预算

- **230+ 个 Python 脚本**（全部通过编译），较之前的 499 个有所增长。
- **118 个定时任务**（约 110 个已启用），较之前的 115 个有所增长。
- **50+ 个 SKILL.md** 文件。
- **82 个标的**，12+ 个数据源。
...
在 v1.5 全部改动完成后，Codex（GPT-5.6）执行了一次完整的工程质量审计：完整阅读全部 19 个被修改的 Python 文件（不只是改动部分）、对全部 19 个文件运行 `py_compile`（全部通过）、对关键文件运行 `importlib.import_module()`（全部导入正常）、验证跨文件依赖关系、检查 `scripts/` 与 `cron/scripts/` 副本的同步状态，并确认冻结模块未被触碰。

| # | 严重程度 | 文件 | Bug | 修复 | 验证 |
|---|----------|------|-----|-----|-------------|
| 1 | 严重 | `codex_analyst_wrapper.py` 第 766 行 | `HERMES_HOME` 未定义 → Pass C 的宏观/情绪数据从未加载（被 except 静默捕获，不崩溃但功能失效） | 改为 `SCRIPTS_ROOT`（第 39 行已定义） | ✅ `_load_engine_c_memory('AVGO')` 现在能返回真实数据 |
...
引入 Pass C → Pass A → Pass B → 综合 的四步流程，强制交叉核验、联合撰写和全部必须完成的关卡机制（见 [ARCHITECTURE_CN.md](ARCHITECTURE_CN.md#3-双引擎多模型分析流程)）。深度分析模板由中文重写为英文（8 段式）。新增成本分析（约 $7/月实际支出 vs $55/月上限）。新增竞品对比矩阵。
...
- 双Pass AI 委员会：Hermes GLM-5.2（基本面）+ Codex GPT-5.6（技术面）
- 模型回退链：GLM-5.2 → GPT-5.6 → DeepSeek V4 Pro → Qwen3 14B（本地）
...
| **v2.0** | **2026-07-12** | **活跃开发中** | **双引擎委员会正式确立（A 40%/B 35%/C 25%），配备委员会作战室决胜机制。又进行两轮联合审计——累计修复 14 个 bug。230+ 个脚本，118 个定时任务（约 110 个已启用），50+ 个 SKILL.md，82 个标的。预算约 $7/月 vs $55/月上限。上线官网 + 技能包 + 仅法定货币支付；移除加密货币打赏箱。** |

v1.0 → v2.0 累计修复：**74+ 项**，历经十个修复/增强周期。

---

*更新日志 v2.0 | 2026-07-12。英文版本见 [CHANGELOG.md](CHANGELOG.md)。*
